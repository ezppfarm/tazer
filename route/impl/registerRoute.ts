...
export default class oAuthRoute implements routeHandler {
  ...
  async handle(
    ...
    const form_error: Record<string, unknown> = {};

    try {
      username = await usernameValidator.parseAsync(
        registerRequest['user[username]'].value
      );
    } catch (err) {
    ...
    try {
      password = await passwordValidator.parseAsync(
        registerRequest['user[password]'].value
      );
    } catch (err) {
      const zodError = err as ZodError;
    ...
    try {
      email = await emailValidator.parseAsync(
        registerRequest['user[user_email]'].value
      );
    } catch (err) {
      const zodError = err as ZodError;
    ...
    const registerResult = await insertUser(
      username,
      safeUsername,
      password,
      email,
